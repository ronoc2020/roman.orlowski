...
Tools: IDA Pro, Ghidra, Wireshark, Cuckoo Sandbox`
  ...
  if (submitAssessmentBtn) {
    submitAssessmentBtn.addEventListener('click', function() {
      const responses = Array.from(document.querySelectorAll('.assessment-response')).map(textarea => textarea.value);
      const analysis = analyzeRecruiterResponses(responses);
      displayAssessmentResults(analysis);
    });
  }

  function analyzeRecruiterResponses(responses) {
    let redFlags = [];
    let ethicalScore = 0;
    let consistencyScore = 0;
    let problemSolvingScore = 0;

    // Keywords for ethical alignment, consistency, and problem-solving
    const ethicalKeywords = ['etyka', 'uczciwość', 'prawo', 'zasady', 'odpowiedzialność', 'transparentność', 'moralność', 'integritet'];
    const inconsistencyKeywords = ['zmienne', 'niepewne', 'niekonsekwentne', 'sprzeczne', 'wahanie'];
    const problemSolvingKeywords = ['analiza', 'rozwiązanie', 'strategia', 'planowanie', 'decyzja', 'skuteczność', 'metodyka', 'podejście'];
    const negativeKeywords = ['unikanie', 'ignorowanie', 'ukrywanie', 'naruszanie', 'nieprzestrzeganie', 'korupcja', 'oszustwo', 'konflikt interesów'];

    responses.forEach((response, index) => {
      const lowerResponse = response.toLowerCase();

      // Ethical Alignment
      const ethicalMatches = ethicalKeywords.filter(keyword => lowerResponse.includes(keyword)).length;
      const negativeMatches = negativeKeywords.filter(keyword => lowerResponse.includes(keyword)).length;
      if (ethicalMatches > 0 && negativeMatches === 0) {
        ethicalScore++;
      } else if (negativeMatches > 0) {
        redFlags.push(`Pytanie ${index + 1}: Potencjalne problemy etyczne lub unikanie odpowiedzialności.`);
      }

      // Consistency (simplified: look for self-contradiction or vague language)
      const inconsistencyMatches = inconsistencyKeywords.filter(keyword => lowerResponse.includes(keyword)).length;
      if (inconsistencyMatches > 0) {
        redFlags.push(`Pytanie ${index + 1}: Odpowiedź może być niespójna lub niejasna.`);
      } else if (response.length < 30 && index !== 2) { // Short answers might indicate avoidance, except for criticism question
        redFlags.push(`Pytanie ${index + 1}: Odpowiedź jest zbyt krótka, co może wskazywać na unikanie tematu.`);
      }

      // Problem Solving
      const problemSolvingMatches = problemSolvingKeywords.filter(keyword => lowerResponse.includes(keyword)).length;
      if (problemSolvingMatches > 0) {
        problemSolvingScore++;
      } else if (index === 1) { // Specific to the 
problem-solving question
        redFlags.push(`Pytanie ${index + 1}: Brak wyraźnego opisu podejścia do rozwiązywania problemów.`);
      }

      // Lie detection (simplified: look for extreme statements or lack of detail where expected)
      if ((lowerResponse.includes('zawsze') || lowerResponse.includes('nigdy')) && response.split(' ').length < 10) {
        redFlags.push(`Pytanie ${index + 1}: Użycie absolutnych stwierdzeń bez szczegółów może wskazywać na próbę manipulacji.`);
      }
    });

    let overallVerdict = 'Analiza zakończona. Brak znaczących czerwonych flag.';
    if (redFlags.length >= 3) {
      overallVerdict = 'Analiza wykryła potencjalne czerwone flagi. Wymagana dalsza weryfikacja.';
    } else if (redFlags.length > 0) {
      overallVerdict = 'Analiza wykryła drobne nieścisłości. Warto dopytać.';
    }
  ...
  function displayAssessmentResults(analysis) {
    assessmentResultsDiv.innerHTML = '';
    assessmentResultsDiv.style.display = 'block';

    const resultHeader = document.createElement('h4');
    resultHeader.textContent = 'Wyniki Analizy Rekrutacyjnej:';
    resultHeader.style.color = 'var(--neon-yellow)';
    resultHeader.style.marginBottom = '1rem';
    assessmentResultsDiv.appendChild(resultHeader);

    const verdictPara = document.createElement('p');
    verdictPara.textContent = `Werdykt: ${analysis.overallVerdict}`;
    verdictPara.style.color = analysis.redFlags.length >= 3 ? 'var(--critical)' : (analysis.redFlags.length > 0 ? 'var(--neon-orange)' : 'var(--pip-green)');
    verdictPara.style.fontWeight = 'bold';
    assessmentResultsDiv.appendChild(verdictPara);

    if (analysis.redFlags.length > 0) {
      const redFlagsList = document.createElement('ul');
      redFlagsList.style.marginTop = '1rem';
      redFlagsList.style.color = 'var(--critical)';
      redFlagsList.innerHTML = '<li>**Czerwone Flagi:**</li>';
      analysis.redFlags.forEach(flag => {
        const li = document.createElement('li');
        li.textContent = flag;
        redFlagsList.appendChild(li);
      });
      assessmentResultsDiv.appendChild(redFlagsList);
    }

    const scoreDetails = document.createElement('div');
    scoreDetails.style.marginTop = '1.5rem';
    scoreDetails.innerHTML = `
      <p>Ocena Etyczna: ${analysis.ethicalScore} / ${responses.length}</p>
      <p>Ocena Spójności: ${analysis.consistencyScore} / ${responses.length}</p>
      <p>Ocena Rozwiązywania Problemów: ${analysis.problemSolvingScore} / ${responses.length}</p>
    `;
    assessmentResultsDiv.appendChild(scoreDetails);

    // Add a note about the nature of the assessment
    const disclaimer = document.createElement('p');
    disclaimer.style.marginTop = '1.5rem';
    disclaimer.style.fontSize = '0.8rem';
    disclaimer.style.color = 'rgba(255, 255, 255, 0.6)';
    disclaimer.textContent = 'Uwaga: Ta analiza jest uproszczoną symulacją i nie zastępuje profesjonalnej oceny HR.';
    assessmentResultsDiv.appendChild(disclaimer);
  }
